// Site-level override of themes/DoIt/assets/js/theme.ts (pinned at submodule commit 4a3ebea1).
// Patch: HTML-escape the user query before it is rendered in the search
// "no results" template. Typeahead renders templates via innerHTML, so an
// unescaped query is an XSS sink. Upstream (HEIGE-PCloud/DoIt) is still
// vulnerable — re-apply this patch on top of a new theme.ts copy whenever the
// submodule is updated and upstream has not fixed it yet.
// Everything else in this file is an unmodified copy of the theme source.

export {};

// Escape a string for safe interpolation into HTML rendered via innerHTML.
...
function initSearch() {
    ...
    const isMobile =
        window.matchMedia && window.matchMedia("only screen and (max-width: 680px)").matches;
    ...
    const termFrequency = searchConfig.termFrequency ? searchConfig.termFrequency : 1.0;
    const pageLength = searchConfig.pageLength ? searchConfig.pageLength : 0.75;
    const termSimilarity = searchConfig.termSimilarity ? searchConfig.termSimilarity : 1.0;
    const termSaturation = searchConfig.termSaturation ? searchConfig.termSaturation : 1.4;
        ...
        const autosearch = autocomplete(
            ...
            },
            {
                ...
                templates: {
                    ...
                    empty: ({ query }) =>
                        `<div class="search-empty">${searchConfig.noResultsFound}: <span class="search-query">"${escapeHTML(query)}"</span></div>`,
                    footer: () => {
...
function initLightGallery() {
    if (window.config.lightGallery) {
        lightGallery(
            document.querySelector("article.page.single") || document.getElementById("content"),
...
function initToc() {
    ...
    const isTocStatic =
        window.matchMedia && window.matchMedia("only screen and (max-width: 1000px)").matches;
        ...
        window._tocOnScroll =
            ...
            (() => {
                ...
                if (headerLinkElements.length > 0) {
                    if (
                        headerLinkElements[headerLinkElements.length - 1].offsetTop < INDEX_SPACING
                    ) {
...
function initCodeblocks() {
    document.querySelectorAll(".code-block").forEach((codeBlock) => {
        ...
        const titleButton = codeBlock.querySelector("button.code-block-button") as HTMLDivElement;
