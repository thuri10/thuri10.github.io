...
This is an article of my adventures with golang **gopacket** library. As I was working on a personal project, I came across [gopacket](https://github.com/google/gopacket) library which provides capabilities for decoding packets in Go. <!-- more --> The capabilities provided by the library motivated me to try malware traffic analysis with the library instead of using Wireshark. This article focuses on analyzing the `QKBOT` malware traffic .
...
![Filter GET HTTP Method](packet_name-filter.png)
